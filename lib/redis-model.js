...
RedisModel.prototype.getRandom = function(callback) {
  var self = this;
  var pattern = self.kHash("*");
  self.scanAll(pattern, function(hashKeys) {
    var randomKey = hashKeys[Math.floor(Math.random() * hashKeys.length)];
    var short_url = randomKey.split(':').pop();
...
RedisModel.prototype._scanAll = function (pattern, cursor, returnSet, callback) {
  ...
  self.db.scan(
    ...
    function(err, data) {
      ...
      if(cursor === '0') {
        if(callback) {
          var returnArray = Array.from(returnSet);
