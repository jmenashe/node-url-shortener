import url from 'url';
import RedisModel from './redis-model.js';
...
export default opts => {
  ...
  self.shorten = function (long_url, startDate, endDate, cNew, callback) {
    if (this.checkUrl(long_url, true)) {
      if (!/^(?:f|ht)tps?\:\/\//.test(long_url)) {
        // No protocol specified; prepend http to ensure correct redirect behavior.
        long_url = `http://${long_url}`;
      } 
      this.getModel((err, model) => {
        if (err) {
          callback(500);
        } else {
          let dateObj = startDate ? {'start_date': new Date(startDate), 'end_date' : new Date(endDate)} : {}
  ...
  self.random = function (callback) {
    this.getModel((err, model) => {
      if(err) {
        callback(500);
      } else {
        model.getRandom(callback);
      }
    });
  };
